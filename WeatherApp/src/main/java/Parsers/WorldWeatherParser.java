...
public class WorldWeatherParser {
    // Выдает информацию на неделю V1.0
    public static HashMap<String, String> getWeekForecast_old(String city_name) throws IOException {
        ...
        for (Element day: days){
            ...
            String date_key = date + '.' + Tools.getMonth(month) + '.' + Tools.year_format.format(new Date());
    ...
    public static String getTodayForecast(String city_name) throws IOException {
        String city = Tools.getCity(city_name);

        SimpleDateFormat date_format = new SimpleDateFormat("dd.MM.yyyy");
        String curr_date = date_format.format(new Date());

        String url = "https://world-weather.ru/pogoda/russia/" + city + "/14days/#" +  new SimpleDateFormat("yyyy-MM-dd").format(new Date());
        Document page = Tools.getPage(url);

        if (page.equals(null)){ return null; }

        return getDayForecast(city_name, curr_date);
    }
    ...
    public static String getDayForecast(String city_name, String date) throws IOException {
        ...
        if (date.matches("(\\d+)\\.([0-1][0-9])\\.(\\d{4})")) {
            ...
            final_date = Tools.year_format.format(new Date()) + "-" + Tools.getMonth(date.split("\\s")[1]) + "-" + date.split("\\s")[0];
        ...
        for (Element element : tableWth){
            ...
            str = Tools.year_format.format(new Date()) + "-" + Tools.getMonth(str.split("\\s")[1]) + "-" + str.split("\\s")[0];
    ...
    public static HashMap<String, String> getWeekForecast(String city_name) throws IOException {
        ...
        for (Element day_el: tableWth){
            ...
            String date_key = current_date.split("\\s")[0] + '.' + Tools.getMonth(current_date.split("\\s")[1]) + '.' + Tools.year_format.format(new Date());
    ...
    public static HashMap<String, String> getTwoWeekForecast(String city_name) throws IOException {
        ...
        for (Element element : tableWth){
            ...
            str =  str.split("\\s")[0] + '.' + Tools.getMonth(str.split("\\s")[1]) + '.' + Tools.year_format.format(new Date());
    ...
    public static HashMap<String, String> getMonthForecast(String city_name) throws IOException {
        ...
        String url = "https://world-weather.ru/pogoda/russia/" + city + "/" + month + "-" + Tools.year_format.format(new Date()) + "/";
        System.out.println(url);
        HashMap<String, String> final_res = new HashMap<String, String>();
        Document page = Tools.getPage(url);
